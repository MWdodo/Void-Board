...
Code and layput by Matthew Wrisley
Created 14APR2025
...
HISTORY_FILE = "VOID_1680AM_LOG.txt"
...
class App:
    def __init__(self):
        ...
        pygame.display.set_caption("VOID 1680 AM Board")
        ...
        self.clock_font = pygame.font.SysFont("Arial", 16, bold=True)
    ...
    def _handle_mouse_click(self, pos):
        ...
        if self.caller_button.collidepoint(pos):
            ...
            self._draw_card(self.caller_deck, "caller")
            
        if self.playlist_button.collidepoint(pos):
            self.button_pressed = "playlist"
            self._draw_card(self.playlist_deck, "playlist")
            
        if self.d6_button.collidepoint(pos):
            self.button_pressed = "d6"
            self._roll_d6()
            
        if self.clear_history_rect.collidepoint(pos):
            self.history.clear()
            with open(HISTORY_FILE, 'w', encoding='utf-8') as f:
    ...
    def _draw(self):
        ...
        self._draw_title()
        self._draw_clock()
        self._draw_block_selector()
        self._draw_main_buttons()
        self._draw_result_boxes()
        self._draw_antenna()
        self._draw_on_off_switch()
    ...
    def _draw_main_buttons(self):
        ...
        self._draw_button(self.caller_button, "CALLER", COLORS['caller'], 
        ...
        self._draw_button(self.d6_button, "ROLL D6", COLORS['d6'], 
                         self.d6_button.collidepoint(mouse_pos), 
                         self.button_pressed == "d6")
    ...
    def _draw_result_boxes(self):
        ...
        self._draw_result_box(self.playlist_result_rect, self.playlist_result, 
                            ...
                            COLORS['playlist']['button'])
                            
        self._draw_result_box(self.d6_result_rect, self.d6_result, 
                            COLORS['d6']['result'], 
                            COLORS['d6']['flash'] if self.flash_active and self.flash_target == "d6" else None, 
                            COLORS['d6']['button'])

    def _draw_result_box(self, rect, text, bg_color, flash_color, text_color):
    ...
    def _draw_clock(self):
        ...
        clock_x = self.switch_rect.x + 111
        clock_y = self.switch_rect.y - 92
        clock_width = 115
        clock_height = 40

        clock_rect = pygame.Rect(clock_x - 3, clock_y - 1, clock_width, clock_height)
    ...
    def _draw_card(self, deck, target):
        card = deck.draw()
        
        if card:
            self._flash_result(target, card)
            self.log_history(deck.type, card)
        else:
            self._flash_result(target, "Empty!")
            self.log_history(deck.type, "Empty!")

    def _roll_d6(self):
        value = random.randint(1, 6)
        self._flash_result("d6", f"🎲 {value}")
